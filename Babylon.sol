...
contract Babylon is IBEP20, Auth {
    ...
    uint256 minHoldingDaysUpperLimit = 360 ; //~ 1 Year
    uint256 sellTaxDayFeeUpperLimit = 5 ;    // 0.05% per day  [ 0.05% * ( minHoldingDays - AcualHoldingDays ) ]
    //
    bool public sellTaxEnabled = true;    
    ...
    function takeFee(address sender, address receiver, uint256 amount,uint256 holdingDays) internal returns (uint256) {
         // default buy tax
         uint256 feeAmount = amount.mul(totalFee).div(feeDenominator);
         //Check Selling
         if (receiver == pair) {
           //only if enabled
           if ( sellTaxEnabled && holdingDays < minHoldingDays ) {
            uint256 _remDays  =  minHoldingDays.sub(holdingDays);
            feeAmount = amount.mul(totalFee.add(sellTaxDayFee.mul(_remDays))).div(feeDenominator);
           }
         }   
         //
